...
- 用户存储配额管理
- 管理员后台（仪表盘/图片管理/用户管理/系统设置/API 文档/操作日志）

### 自动采集
- 内置多个随机图片 API 源，自动采集公开图片
- 管理员可自定义添加/编辑/删除采集源
- 支持 redirect（302 跳转）、json（JSON 解析）、direct（直接图片）三种响应类型
- 一键探测 API 响应格式，自动识别类型和 JSON 路径

### 公开 API
- `GET /api/random` — 随机图片接口，302 跳转到随机公开图片
